...
class AdminController extends Controller
{
    ...
    public function allAdmin(){
        ...
        return view('admin.all',compact('admins','pageTitle'));
    }
    ...
    public function adminCreate(Request $request){
        ...
        $usernameExist = Admin::where('username',$request->username)->first();
        if($usernameExist){
            $notify[] = ['error','Username must be unique'];
            return back()->withNotify($notify);
        }

        $emailExist = Admin::where('email',$request->email)->first();
        if($emailExist){
    ...
    public function adminUpdate(Request $request,$id){
        ...
        if ($id == 1) {
            $notify[] = ['error','Sorry! you couldn\'t delete the main admin'];
            return back()->withNotify($notify);
        }

        $usernameExist = Admin::where('id','!=',$id)->where('username',$request->username)->first();
        if($usernameExist){
            $notify[] = ['error','Username must be unique'];
            return back()->withNotify($notify);
        }

        $emailExist = Admin::where('id','!=',$id)->where('email',$request->email)->first();
        if($emailExist){
            $notify[] = ['error','Email must be unique'];
            return back()->withNotify($notify);
        }
