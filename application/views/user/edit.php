          ...
          <div class="col-md-offset-1 col-md-9">
            <form class="form-horizontal" method="post" enctype="multipart/form-data" action="<?php echo base_url('user/update_user_record/'); ?><?php echo $userId; ?>">
              <div class="box-body">
                ...
                <?php $userName = set_value('userName') == false ? $userInfo->userName : set_value('userName'); ?>
                <div class="form-group">
                  <label for="userName" class="col-sm-2 control-label">User-name</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="userName" value="<?php echo $userName; ?>" id="userName" placeholder="User-name">
                  </div>
                </div>
                ...
                <div>
                  ...
                  $district_ids = explode(",", $userInfo->district_ids);
                  $query = "SELECT * FROM district GROUP BY region";
                  ...
                  foreach ($regions as $region) { ?>
                    ...
                    foreach ($districts as $district) { ?>
                      <input <?php if (in_array($district->districtId, $district_ids)) { ?>checked <?php } ?> class="ditrict_id" type="checkbox" id="" name="" value="<?php echo $district->districtId ?>">
                  <?php echo $district->districtTitle . ", <span style=\"margin-left:10px;\"></span> ";
                    }
                  }
                  ?><br />
                  <div id="respose"></div>
                ...
                <script>
                  $(".ditrict_id").change(function() {
                    var district_ids = new Array();
                    $(".ditrict_id").each(function() {
                      if ($(this).is(':checked')) {
                        district_ids.push($(this).val());
                        //console.log(district_ids.toString());
                      }
                    });
                    $("#district_ids").val(district_ids.toString());
                    //district_ids = district_ids.toString()
                    // alert(district_ids);
                    $.ajax({
                        method: "POST",
                        url: "<?php echo site_url('user/assign_district_ids'); ?>",
                        data: {
                          user_id: "<?php echo $userInfo->userId; ?>",
                          district_ids: district_ids,
                        },
                      })
                      .done(function(respose) {
                        $('#respose').html(respose);
