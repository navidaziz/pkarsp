                        ...
                        <tr>
                          ...
                          <td>
                            <select name="vehicle_model_year" id="vehicle_model_year" required>
                              ...
                              <?php for ($years = 2022; $years >= 1950; $years--) { ?>
                                <option value="<?php echo $years; ?>"><?php echo $years; ?></option>
                              <?php } ?>
                            </select>
                          </td>
                          <td><input class="form-control"  style="width: 100% !important;"  type="text" id="type_of_vehicle" name="type_of_vehicle" required></td>
                          <td><input class="form-control"  style="width: 100% !important;"  type="date" id="expiry_of_fit_certificate" name="expiry_of_fit_certificate" required></td>
                          <td><input class="form-control" min="5" max="100" style="width: 100% !important;" title="10" type="number" id="total_seats" name="total_seats" required></td>
                          <td>
                            <script>
...
<div class="jumbotron" style="padding:7px; margin:0px">
  <h4>CBS Registration Form</h4>
  <p>Please Submit Character Bulding Society Data Online</p>
  <p><a target="new" class="btn btn-primary btn-small" href="<?php echo site_url("cbs_reg_form"); ?>" role="button">CBS Online Form</a>
  <a target="_blank" class="btn btn-link btn-small" href="https://psra.gkp.pk/uploads/Establishment_of_CBS.pdf">Download CBS Notification / Guideline</a>
  </p>
</div>
</div>
              ...
              <div style="text-align: center;">
                <button class="btn btn-success" style="margin:5px" data-toggle="modal" data-target="#vehicles">Please Submit School Vehicles Information</button>
              </div>
        ...
        <div class="modal fade" id="MrVaccinated" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title pull-left" id="exampleModalLabel">MR Vaccination Report</h5>
                <button type="button" class="close pull-right" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
                <form onsubmit="return confirm('Are you sure you want to continue with this MR vaccinated aata.');" id="mrForm" action="<?php echo base_url('Temp_controller/add_mr_vaccinated_report'); ?>" method="post">
                  ...
                  <h4>MR Vaccination Report of Students (Age 15 or less)</h4>

                  <table class="table">
                    <tr>
                      <th>Total Students Enrollment In School (Age 15 or less)</th>
                      <th>Total Students MR Vaccinated (15 or less)</th>
                    ...
                    <tr>
                      ...
                      <th><input class="form-control" style="width: 100% !important;" min="2021-10-14" type="date" id="date" name="vaccinated_date" required></th>
                    </tr>
                    <tr>
                  ...
                  <table class="table">
                    <tr>
                      <th>S/No</td>
                      <th>Total Enrolled Students (Age 15 or less)</th>
                      <th>Total MR Vaccinated Students (Age 15 or less)</th>
                      <th>Dated</th>
                      <th>Remarks</th>
                    </tr>
        ...
        <div style="text-align: center; display:none">
          <button class="btn btn-success" style="margin:5px" data-toggle="modal" data-target="#MrVaccinated">Please Submit MR Vaccination Report of Students (Age 15 or less)</button>
        </div>
      ...
      <div class="form-popup" id="minorityFrom">
        <form  class= "form-container" class="form-container" action="<?php echo base_url('Temp_controller/save_minority_students');?>" method="post">
          ...
          <div class="form-group">
            <h5 style="padding-left: 10px"><strong>Please Submit Minority (Non-Muslim) Students Data From Class 1 to 10</strong></h5>
          <label class="control-label col-sm-2" for="total_students">Total Students (Muslim + Non-Muslim)</label>
          <input class="col-sm-2" type="number" min="1"  name="total_students" required>
          <label class="control-label col-sm-2" for="total_minority_students">Total Number of Minority (Non-Muslim) Students</label>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <form onsubmit="return confirm('Are you sure you want to continue with this COVID-19 Vaccination Data.');" id="mrForm" action="<?php echo base_url('Temp_controller/temp_student_covid'); ?>" method="post">
                        ...
                        <div style=" font-size: 12px; border:1px solid #9FC8E8; border-radius: 10px; min-height: 50px;  margin: 10px; padding: 10px; background-color: white; text-align:center">
                        <h4>Now, Add Detail of Vaccinated Students. <br /><a class="btn btn-danger" href="<?php echo site_url('covid_vaccination_students'); ?>" target="_new" > Add Students Detail</a> </h4>
                        </div>
...
<?php $this->session->flashdata("msg");  ?>
     ...
     <div class="col-md-6 col-md-offset-3 col-sm-12">
            <ul class="list-group" style="text-align: center;">
              ...
              //get which session school registered
              $query = "SELECT MIN(session_year_id) as reg_session_id from school WHERE schools_id='" . $schooldata->schoolId . "'";
              $registration_session_id = $this->db->query($query)->row()->reg_session_id;

              $query = "SELECT * FROM `session_year` WHERE sessionYearId>='" . $registration_session_id . "'
              AND DATE(session_start) <= '".date('Y-m-d')."'
              ";
              ...
              foreach ($sessions as $session) {
                ...
                $query = "SELECT COUNT(*) as total, schoolId, `status` FROM school  
                          WHERE session_year_id = '" . $session->sessionYearId . "'
                          AND schools_id='" . $schooldata->schoolId . "'";
                $applied_session = $this->db->query($query)->row();



                if ($applied_session->total) {
                  echo '<li class="list-group-item ';
                  if ($applied_session->status == 1) {
                    echo "list-group-item-success";
                  }
                  if ($applied_session->status == 0) {
                    echo "list-group-item-danger";
                  }
                  if ($applied_session->status == 2) {
                    echo "list-group-item-light";
                  }
                  echo ' ">';
              ?>
                  <h4>
                    <a href="<?php echo base_url("school/explore_school_by_id/" . $applied_session->schoolId . ""); ?>">
                      Session <?php echo $session->sessionYearTitle ?>
                      <?php if ($applied_session->status == 0) {
                        echo '<i class="fa fa-wpforms" aria-hidden="true"></i>';
                        echo "<br /><small><i>Online application submission pending.</i></small>";
                      }
                      if ($applied_session->status == 1) {
                        echo '<i class="fa fa-check"></i>';
                        echo "<br /><small><i>Completed.</i></small>";
                      }
                      if ($applied_session->status == 2) {
                        echo '<i class="fa fa-spinner" aria-hidden="true"></i>';
                        echo "<br /><small><i>Application inprogress.</i></small>";
                      }
                      ?>
                    </a>
                  </h4>
                  ...
                  if ($next_apply_button == 1) {
                    echo '<li class="list-group-item  list-group-item-warning ">';
                  ?>
                    <h4>
                      Apply for session <?php echo $session->sessionYearTitle ?>
                      <a class="btn btn-primary" href="javascript:void(0);" onclick='load_form_in_modal("<?php echo $session->sessionYearId; ?>", "Now Apply for session <?php echo $session->sessionYearTitle ?>", "school/apply_for_session");'>
                        Apply</i>
                      </a>
                    </h4>

                  <?php } else {
                    echo '<li class="list-group-item  list-group-item-dark disabled ">';
                  ?>
                    <h4>
                      Apply for session <?php echo $session->sessionYearTitle ?>
                    </h4>
...
<!-- 2nd code here ... -->
  <div class="col-md-6 col-md-offset-3 col-sm-12">
    <?php
          ...
          $application_status = "";
          $query = "SELECT COUNT(school.schoolId) as total, school.schoolId, school.`status` , session_year.sessionYearTitle
          FROM school 
          INNER JOIN session_year ON(session_year.sessionYearId = school.session_year_id) 
    WHERE  schools_id='" . $schooldata->schoolId . "'
    AND school.schoolId = '" . $school_id . "'";
          $applied_session = $this->db->query($query)->row();

          if ($applied_session->status == 1) {
            $application_status = "list-group-item-success";
          }
          if ($applied_session->status == 0) {
            $application_status = "list-group-item-danger";
          }
          if ($applied_session->status == 2) {
            $application_status = "list-group-item-light";
          }

    ?>
    <ul class="list-group" style="text-align: center;">
      <li class="list-group-item <?php echo $application_status; ?> ">
        <h4>
          <a href="<?php echo base_url('school/explore_school_by_id/') ?><?php echo $school_id; ?>">
            Registration for Session <?php echo $applied_session->sessionYearTitle; ?>
            <?php if ($applied_session->status == 0) {
              echo '<i class="fa fa-wpforms" aria-hidden="true"></i>';
              echo "<br /><small><i>Online application submission pending.</i></small>";
            }
