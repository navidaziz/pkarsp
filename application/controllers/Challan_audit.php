...
class Challan_audit extends CI_Controller
{
   ...
   public function index()
   {
      ...
      $this->data['title'] = 'Challen Audit';
      $this->data['description'] = 'Challan Audit';
      $this->data['view'] = 'challan_audit/challan_list';
   ...
   public function get_bank_challan_list()
   {
      ...
      $this->load->view('challan_audit/bank_challan_list', $this->data);
   }
   ...
   public function verified_challans()
   {
      $verified_challans = $this->input->post('verified');
      $response = 'error';
      foreach ($verified_challans as $challan_id => $verified_challan) {
         var_dump($verified_challan['v']);
         $input['verified_by'] = $this->session->userdata('userId');
         $input['verified_date'] = date("Y-m-d h:i:s");
         $input['verified'] = $verified_challan['v'];

         //if ($this->input->post("bise_verified")) {
         //	$input['bise_tdr'] = (float) $this->input->post("bise_tdr");
         //}
         //$input['bise_tdr'] = (float) $this->input->post("bise_tdr");
         $this->db->where('bank_challan_id', $challan_id);
