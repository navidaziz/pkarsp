...
class Bank_challan_entry extends Admin_Controller
{
   ...
   public function get_bank_challan_list()
   {
      ...
      WHERE DATE(`bank_challans`.`created_date`) = DATE(NOW()) 
      ORDER BY bank_challans.bank_challan_id DESC";
   ...
   public function add_stan_number()
   {
      ...
      if ($count == 0) {
        ...
        VALUES (
         ...
         $user_stants = $this->db->query($query)->result();
         foreach ($user_stants as $user_stant) {
            if ($user_stant->old_challan_id) {
               echo "STAN Used Detail:\n";
               $query = "SELECT * FROM `old_challans` WHERE `old_challans`.`id`='" . $user_stant->old_challan_id . "'";
               $old_challan = $this->db->query($query)->row();
               if ($old_challan->file_source == 'excel Manual Receipts') {
                  echo "Excel File: Sheet excel Manual Receipts, Serial No: " . $old_challan->excel_s_no . "\n";
               } else {
                  echo "Excel File: Sheet File After 31-5, Serial No: " . $old_challan->excel_s_no . "\n";
               }
            } else {
               echo "STAN Used By:\n";
               $query = "SELECT schools.schoolName, schools.schoolId, challan_for FROM bank_challans 
               INNER JOIN schools ON (schools.schoolId = bank_challans.schools_id)
               WHERE challan_no = '" . $statn_number . "'";
               $new_challan = $this->db->query($query)->row();
               echo $new_challan->schoolName . " (SID: " . $new_challan->schoolId . ") for session ";
               $query = "SELECT sessionYearTitle FROM `session_year` WHERE sessionYearId='" . $new_challan->session_id . "'";
               $session_detail = $this->db->query($query)->row();
               echo $session_detail->sessionYearTitle . " ";
               echo "(" . $new_challan->challan_for . ")";
            }
         }
      }
      //$variables = $_POST;
