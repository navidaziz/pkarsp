...
class Form extends MY_Controller
{
	...
	public function section_b($session_id)
	{
		...
		$this->check_school_session_entry($session_id, $this->data['school']->schools_id,  $this->data['school']->reg_type_id);
	...
	private  function check_school_session_entry($session_id, $school_id, $reg_type_id)
	{
		echo $query = "SELECT * FROM school 
		          WHERE schools_id = '" . $school_id . "' 
				  AND session_year_id= '" . $session_id . "'
				  AND reg_type_id = '" . $reg_type_id . "'";
		...
		if (!$school_session_entry) {
			...
			var_dump($school_session_detail);
			exit();
	...
	public function print_renewal_upgradation_bank_challan($session_id)
	{
		$this->data['session_id'] = $session_id = (int) $session_id;
