...
FROM
  ...
  public function get_school_username_password()
  {
    ...
    if ($schools_id) {
      $q = "SELECT `schools`.`schoolId`,`schools`.`registrationNumber`,`schools`.`schoolName`,
                `schools`.`schoolMobileNumber`,`schools`.`yearOfEstiblishment`,
                `users`.`userTitle`,`users`.`userName`,`users`.`userPassword`,
                `users`.`contactNumber`, `users`.`cnic`  
                FROM `schools` inner join users on `schools`.`owner_id`=`users`.`userId` 
  ...
  public function apply_session()
  {
    ...
    if ($schools_id) {
      ...
      echo "You are not allow to apply. please login using school account and apply.";
      exit();
    }
  ...
  public function fetch_previous_data($new_session_school_id, $schools_id)
  {



    $school_inserted_id = (int) $new_session_school_id;
    $schools_id = (int) $schools_id;

    $query = "SELECT session_year_id FROM school 
               WHERE schoolId='" . $new_session_school_id . "' 
              AND schools_id = '" . $schools_id . "'
              AND status = 0";
    $school_current_session = $this->db->query($query)->row();

    if ($school_current_session) {
    } else {
      echo "Something went wrong.";
      exit();
    }

    $school_current_session_id = ($school_current_session->session_year_id - 1);

    //get previous session data...
    $query = "SELECT * FROM school WHERE session_year_id='" . $school_current_session_id . "' and schools_id = '" . $schools_id . "'";
    $school_session = $this->db->query($query)->row();
    ...
    redirect('school/explore_school_by_id/' . $new_session_school_id);
